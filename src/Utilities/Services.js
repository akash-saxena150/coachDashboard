...
axios.interceptors.request.use((cfg)=> { 
    let {url} = cfg, localRegion = '';
    if(url.indexOf(config.api.me)>=0)
        localRegion = 'me'
    if(url.indexOf(config.api.in)>=0)
        localRegion = 'in'    
    if(url.indexOf(config.api.row)>=0)
        localRegion = 'row'
    if(url.toLowerCase().indexOf('transactionlist')>=0)
        localRegion = getData('region');
    let authToken = getData('auth');
    authToken = (localRegion && authToken)?(JSON.parse(authToken))[localRegion]:null;
...
var apiCall = function(api, region, cb, efn, props, type, obj){
    ...
    const url = config.api.base + ((api==='transactionlist')?config.api['in']:config.api[localRegion]) + config.api[api];    
    switch(type){
        case 'get':
        axios.get(url)
        .then((data)=>{cb(data, props)})
        .catch((err)=>{handleErr(err, props, efn)})
        break;
        case 'post':
            let tempObj = {};
            if (obj && !obj.header)
...
export {GetHt, GetWd, setData, removeItem, getData, apiCall, croptxt, compareDate, firebaseLogin, formatDate, cmtoinch, lbtokg}